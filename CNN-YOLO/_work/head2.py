...
def skinmask(img):
    ...
    hsvim = cv.cvtColor(img, cv.COLOR_BGR2HSV) #采用HSV策略的肤色检测
    ...
    try:
        ...
        if defects is not None:
            ...
            for i in range(defects.shape[0]):  #计算角度
                ...
                angle = np.arccos((b ** 2 + c ** 2 - a ** 2) / (2 * b * c))  #余弦定理(cosine theorem)
                if angle <= np.pi / 2:  #判断是否满足小于90°或小于pi/2,小于即认为是一个手指
                    cnt += 1
