...
class general_pose_model(object):
    ...
    def get_hand_model(self, modelpath):
        ...
        prototxt   = os.path.join(modelpath, "handpose_deploy.prototxt")
    ...
    def vis_pose(self, imgfile, points):
        ...
        for idx in range(len(points)):
            if points[idx]:
                cv2.circle(img_cv2_copy, points[idx], 8, (0, 255, 255), thickness=-1,lineType=cv2.FILLED)
                cv2.putText(img_cv2_copy, "{}".format(idx), points[idx], cv2.FONT_HERSHEY_SIMPLEX,1, (0, 0, 255), 2, lineType=cv2.LINE_AA)
